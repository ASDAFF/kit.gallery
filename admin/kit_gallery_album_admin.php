...
$sTableID = "tbl_kit_gallery_album";
...
if($parent_id > 0)
{
	if($ar_nav = CKitGallerySection::GetByID($parent_id))
	{
		...
		$sSectionUrl = BX_ROOT."/admin/kit_gallery_album_admin.php?lang=".urlencode(LANG)."&find_parent_id=".$parent_id;
		$chain->AddItem(array(
...
<script type="text/javascript" src="<?=BX_ROOT?>/js/kit.gallery/jquery.fineuploader-3.7.1.min.js"></script>
<link href="<?=BX_ROOT?>/js/kit.gallery/style/fineuploader.css" rel="stylesheet">
...
<?CJSCore::Init(array('kit_gallery_image_upload_handler'));?>
